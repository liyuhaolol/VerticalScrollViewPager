...
import cn.lyh.spa.ptr.util.PtrCLog;
 ...
 * It defines {@link cn.lyh.spa.ptr.PtrUIHandler}, which allows you customize the UI easily.
 */
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (A.getInstance().a != 1){
                    ...
                    if (mIsVertical != null && mIsVertical){
                        mLastMoveEvent = e;
                        ...
                        if (DEBUG) {
                            ...
                            PtrCLog.e(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown);
                        }
...
/*    @Override
    ...
    private boolean judgeScrollDirection(float dx, float dy) {
        ...
        if (Math.abs(dx) > 10 && Math.abs(dx) > 2 * Math.abs(dy) ) {
            mIsVertical = false;
        } else if (Math.abs(dy) > 10 && Math.abs(dy) > 2 * Math.abs(dx)) {
            mIsVertical = true;
        } else {
            canJudge = false;
